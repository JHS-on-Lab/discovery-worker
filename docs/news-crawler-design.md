...
키워드 기반으로 여러 포털의 뉴스 탭을 검색해, 검색된 기사의 **URL·제목·본문·메타데이터**를 수집·저장하는 서비스다. 단발 스크립트가 아니라 운영(operation)을 전제로 한다.

- **대상 소스**: 네이버 뉴스, 다음 뉴스, 구글 뉴스, 웨이보 (`portal_type`: `NAVER`, `DAUM`, `GOOGLE`, `WEIBO`)
...
본문 전문이 필수인데, 어떤 포털도 공식 API로 본문 전문을 제공하지 않는다(네이버 검색 API는 제목·요약·링크·날짜만, 다음/카카오는 뉴스 전용 검색 API 자체가 없음, 웨이보 검색 API는 일반 개발자 미개방). 따라서 **모든 소스를 스크래핑으로 처리**한다. 발견(검색 결과 수집)도, 추출(본문 파싱)도 스크래핑이다. 그 결과 운영의 핵심 난제는 API 쿼터가 아니라 **안티봇 회피와 IP 관리**다.
...
flowchart LR
  ...
  DISP --> DA[Discovery adapters<br/>Naver / Daum / Google / Weibo]
...
- **발견(Discovery)**: 입력 `(keyword, portal_type)` → 출력은 기사 URL을 `article_url`에 `status=discovered`로 적재. 소스별 어댑터가 검색 결과 페이지를 스크래핑한다. 본문은 건드리지 않는다.
- **추출(Extraction)**: `article_url`에서 작업을 점유 → 기사 페이지를 가져와 제목·본문·메타를 파싱 → 성공 시 Sink에 기록하고 `status=stored`.
...
```
news_crawler/
  adapters/            # SourceAdapter 구현: naver.py, daum.py, google.py, weibo.py
...
class SourceAdapter(Protocol):
    ...
    def discover(self, keyword: str, cursor: str | None) -> DiscoverResult:
        """검색 결과를 긁어 기사 URL 목록과 다음 cursor를 반환. 본문은 다루지 않음."""
...
class Extractor(Protocol):
    def extract(self, url: str, html: str, host: str) -> Article | ExtractionFailure:
        """규칙 존재 시 규칙 우선, 없으면 라이브러리 체인. 실패 판정 포함."""

class Sink(Protocol):
    def write(self, article: Article) -> None:
        """현재 FileSink(.jsonl), 나중에 SolrSink. 호출부는 동일."""
```
...
- `--portal` : `naver` | `daum` | `google` | `weibo` | `all` — **점유 쿼리의 `WHERE portal_type` 필터값**. 기본값 `all`.

```bash
python -m news_crawler --role discovery  --portal naver   # 네이버 발견자
python -m news_crawler --role discovery  --portal daum    # 다음 발견자
python -m news_crawler --role extraction --portal all     # 공용 추출자
```

```yaml
# 같은 이미지, 인자만 다르게
services:
  discover-naver: { image: news-crawler:latest, command: ["--role","discovery","--portal","naver"] }
  discover-daum:  { image: news-crawler:latest, command: ["--role","discovery","--portal","daum"] }
  extract:        { image: news-crawler:latest, command: ["--role","extraction","--portal","all"], deploy: { replicas: 3 } }
```
...
**권장 형태**: 발견은 포털별로 분리(스크래핑 대상·차단 양상·렌더링 방식이 포털마다 다름), 추출은 공용(기사 URL이 언론사 사이트라 포털 구분이 무의미). 확장도 병목만 독립적으로 — 네이버 키워드가 많으면 `discover-naver`만 replicas를 늘리고(같은 포털 발견자 여러 개는 `SKIP LOCKED`로 키워드를 안 겹치게 나눠 가짐), 추출이 밀리면 `extract`만 늘린다. 소규모일 땐 `--portal all` 발견자 하나로 시작해 트래픽이 커지면 포털별로 무중단 분리.
...
- **기사 데이터**: JSON Lines(`.jsonl`), 한 줄에 기사 하나. append 친화적이고 나중에 Solr bulk import에 그대로 쓸 수 있다.
- **파티셔닝**: `data/{YYYY-MM-DD}/{portal_type}.jsonl` — 날짜·소스별로 나눠 관리·재적재를 조각 단위로.
- **운영 로그**: 수집 진행·하트비트·에러는 기사 데이터와 섞지 않고 별도 로그로 분리한다. 정보 로그와 **전용 에러 로그**를 또 나눈다 — 상세는 12절.
...
- 결과: **하나의 기사는 한 번만 추출되고 Sink에 한 번만 기록된다.** Sink가 파일이든 Solr든 동일하게 동작한다.

**URL 정규화 필수.** 해시 전에 정규화하지 않으면 같은 기사가 다른 URL로 보여 중복 제거가 안 먹는다. 정규화 규칙:
...
발견의 목표는 "검색 결과에서 새 기사 URL만 추려 큐에 넣기"다. 핵심 난제는 **어디까지 긁고 멈출 것인가**이며, 이는 소스가 제공하는 정렬·기간 필터 기능에 따라 갈린다.
...
**증분 중단**("이미 본 URL을 만나면 그 뒤는 다 오래된 것이니 멈춤")은 결과가 **최신순 정렬**일 때만 성립한다. 요즘 포털 검색은 기본이 정확도순이라 신·구 기사가 섞여 나올 수 있어, 정렬을 신뢰할 수 없으면 증분 중단은 새 기사를 놓치게 한다. 그래서 두 경로로 나눈다.

- **정렬 신뢰 가능(최신순 강제 가능)**: 기간 필터 + 최신순 + **증분 중단**(이미 본 url_hash 또는 컷오프보다 오래된 기사를 만나면 그 지점에서 페이징/스크롤 종료). 값싸게 끝난다.
- **정렬 신뢰 불가(정확도순뿐)**: 기간 필터로 **결과 집합을 한정**한 뒤, 정렬을 믿지 않고 그 유한 집합을 끝까지 훑으며 기사마다 날짜로 판정해 컷오프보다 새것만 담는다. 멈춤 기준은 "헌 기사를 만나서"가 아니라 "필터로 좁혀진 결과를 다 봤거나 페이지/스크롤 상한 도달". 집합이 작으니 끝까지 봐도 부담이 적다.
...
웨이보는 뉴스가 아니라 소셜 포스트라 별도 분석이 필요하다. 확인 항목: ① 기간 필터 지원 여부·단위, ② 최신순 정렬 가능 여부, ③ 검색 대상 범위(공식 계정 글만 vs 전체 포스트), ④ 로그인·해외 접속 차단 대응(발견 단계부터 headless + 세션 유지가 필요할 가능성이 높음). ①·② 결과에 따라 네이버형(증분 중단) 또는 구글형(집합 한정)으로 떨어진다.
...
언론사 HTML이 제각각이라 초반에는 라이브러리가 대부분을 처리하고, 규칙은 실패가 잦은 소수 도메인에만 쌓인다.
...
- **규칙 핫리로드와의 시너지**: 특정 언론사 때문에 실패가 쌓였을 때, 관리 UI에서 그 도메인 규칙을 등록(즉시 반영)하고 실패 URL을 재투입하면 워커가 새 규칙으로 다시 긁는다.
...
기사마다 `extraction_method`와 본문 길이를 기록하고, `domain`의 `success_rate`·`avg_body_len`을 갱신한다. 특정 도메인의 성공률/평균 본문 길이가 급락하면 알림 → "이 도메인 규칙을 손볼 때"라는 신호 → 관리 UI에서 규칙 수정 → 핫리로드로 자동 반영. 루프가 닫힌다.
...
**4. 추출 코어.** 라이브러리 체인으로 본문 추출 + 성공/실패 판정 + 실패 분류. **저장해둔 기사 HTML 픽스처로 개발**(네트워크 없이 파서만 빠르게 반복; Fetcher와 분리돼 있다는 게 모듈화의 증거).
...
→ 검증: 큐에 URL 몇 개 넣고 워커를 돌리면 파일에 기사가 쌓이고, 실패한 건 상태가 바뀌는가.
...
- **검색 결과 로딩 방식 실측.** 각 포털 검색 결과를 정적 요청으로 받아 기사 링크가 HTML에 다 들어있는지 확인한다. 무한 스크롤·"더보기"·동적 로딩이면 정적으로는 일부만 잡힌다. 이 실측이 발견 어댑터를 static으로 짤지 headless로 짤지를 가르며, 무한 스크롤이면 내부 요청 직접 호출 가능 여부(8.5)도 함께 확인한다.
- **발행일시 정규화 규칙.** 포털 검색 결과의 날짜와 기사 페이지의 날짜가 다를 수 있고, 타임존·상대표기("3시간 전")가 섞인다. 추출 시 발행일시를 어떤 기준으로 정규화할지 미리 정한다(8절 컷오프 판정의 정확도와 직결).
- **"본문"의 경계 기준.** 기자명·소속·사진 캡션·관련기사·구독 안내를 본문에 포함할지 제외할지 일관 기준을 정한다(규칙 작성과 라이브러리 보정의 기준이 됨).
- **테스트 픽스처 확보.** 개발 중 실제 포털을 반복 호출하면 그 단계에서 IP가 차단될 수 있다. 검색 결과·기사 페이지 HTML을 몇 개 저장해두고 파서를 그 위에서 개발하면 네트워크 없이 빠르게 반복하고 차단도 피한다(4단계 전제).
