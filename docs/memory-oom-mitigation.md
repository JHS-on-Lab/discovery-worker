...
- 6·7번 적용 후 서버 mem 로그로 실제 피크 감소 확인 (아직 실측 전 — 사용자가 직접 배포해 확인 예정)
...
- rss 폴백 자체가 봇 차단 회피 수단인데 메모리 리스크의 진앙이라는 구조적 모순은 여전함 — 6·7번으로도 부족하면 CDP 기반 조기 차단(§6 참고) 또는 리전/프로필 분리 등 추가 완화 검토
