...
**추가로 검토 중인 더 근본적인 방향**: 지금은 `driver.get(cbmi_url)` 후 리다이렉트가 커밋(=목적지 renderer 프로세스가 이미 할당된 시점)될 때까지 기다리는 구조라, 목적지 페이지가 자기 JS(광고 포함)를 실행하기 시작하기 전에 완전히 끊는 건 아니다. CDP `Network.requestWillBeSent` 이벤트로 리다이렉트 목적지 URL이 요청되는 순간을 캡처하고 즉시 `Page.stopLoading()`으로 중단하면, 광고 도메인 차단보다 한 단계 더 이르게(목적지 페이지의 JS 실행 자체를 안 주고) 끊을 수 있다 — 다만 CDP 이벤트 리스닝이 추가돼 구현 복잡도가 올라가서, 4~6번 효과를 서버에서 먼저 실측하고 그래도 부족하면 착수하기로 함.

## 남은 작업

- 4~6번 적용 후 서버 mem 로그로 실제 피크 감소 확인 (아직 실측 전 — 사용자가 직접 배포해 확인 예정)
...
- rss 폴백 자체가 봇 차단 회피 수단인데 메모리 리스크의 진앙이라는 구조적 모순은 여전함 — 4~6번으로도 부족하면 CDP 기반 조기 차단(위 참고) 또는 리전/프로필 분리 등 추가 완화 검토
